...
export default function GoogleMapsStyleRoute() {
  ...
  // Locations on map (approximate coordinates in percentage)
  const locations = [
    { id: "start", x: 50, y: 50, isStart: true, capacity: null, order: null },
    { id: "loc1", x: 30, y: 30, isStart: false, capacity: 3, order: 1 },
    { id: "loc2", x: 70, y: 25, isStart: false, capacity: 2, order: 2 },
    { id: "loc3", x: 75, y: 60, isStart: false, capacity: 1, order: 3 },
    { id: "loc4", x: 35, y: 70, isStart: false, capacity: 4, order: 4 },
    { id: "loc5", x: 20, y: 55, isStart: false, capacity: 2, order: 5 },
  ]
  ...
  const getOptimalRoute = () => {
    ...
    const deliveryLocations = locations
      .filter((loc) => !loc.isStart && loc.order !== null)
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto rounded-xl overflow-hidden shadow-xl bg-white">
      <div className="aspect-[4/3] relative">
        ...
        <Image src="/google-maps-style-bg.jpg" alt="Google Maps風地図" fill className="object-cover" />
            ...
            >
              ...
              >
                {location.isStart ? (
                  ...
                  location.order && <span className="text-xs font-bold">{location.order}</span>
                )}
              </div>
              {location.isStart ? (
